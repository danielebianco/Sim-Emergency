...
public class Simulatore {
	...
	private int T_ARRIVAL = 15 ; // intervallo di tempo tra pazienti
	...
	private StatoPaziente statoTriage;
	int studi_occupati ;
	...
	private void processEvent(Event e) {
		...
		case CHIAMATA:
			...
			case WHITE:
				queue.add(new Event(e.getOra().plusMinutes(DURATION_WHITE),
						EventType.USCITA, e.getPaziente())) ;
				break;
			case YELLOW:
				queue.add(new Event(e.getOra().plusMinutes(DURATION_YELLOW),
						EventType.USCITA, e.getPaziente())) ;
				break;
			case RED:
				queue.add(new Event(e.getOra().plusMinutes(DURATION_RED),
						EventType.USCITA, e.getPaziente())) ;
				break;
			default:
				break;
		...
		case USCITA:
			...
			// Decidere chi deve essere chiamato (sulla base del colore e di ora di arrivo)
			// Tra i pazienti in attesa
		...
		case TIMEOUT_YELLOW:
			if(e.getPaziente().getStato()==StatoPaziente.YELLOW) {
				...
				queue.add(new Event(e.getOra().plusMinutes(TIMEOUT_RED), 
						EventType.TIMEOUT_RED, e.getPaziente()));
				attesa.add(e.getPaziente()); // ora è in una posizione privilegiata risp a prima
			}
			break;
